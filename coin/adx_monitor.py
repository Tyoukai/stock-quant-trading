...
def calculate_adx(df_local, n):
    ...
    for i in range(n, len(df_local.index)):
        df_local.loc[i]['tr_n'] = df_local.loc[i - 1]['tr_n'] * (n - 1) / n + df_local.loc[i]['tr']
        df_local.loc[i]['dm_plus_n'] = df_local.loc[i - 1]['dm_plus_n'] * (n - 1) / n + df_local.loc[i]['dm_plus']
        df_local.loc[i]['dm_minus_n'] = df_local.loc[i - 1]['dm_minus_n'] * (n - 1) / n + df_local.loc[i]['dm_minus']
    ...
    for i in range(n, len(df_local.index)):
        df_local.loc[i]['adx'] = df_local.loc[i - 1]['adx'] + df_local.loc[i]['dx']
    return df_local
...
def draw(df_local, symbol_local):
    ...
    ax.plot(df_local['start_time'], df_local['di_plus_n'], 'b-,', label='ema')
    ax.plot(df_local['start_time'], df_local['di_minus_n'], 'g-,', label='min')
    ax.plot(df_local['start_time'], df_local['adx'], 'k-,', label='max')
    ax.legend(loc=3)
...
if __name__ == '__main__':
    ...
    for symbol in symbols:
        ...
        # 4、计算DI+ DI- DX ADX
        calculate_adx(fifteen_minute_df, cycle)
        # 7、图形展示
