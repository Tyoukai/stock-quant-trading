...
if __name__ == '__main__':
    ...
    # '20130308', '20150413'
    # '20200701', '20240105'
    stock_df = get_daily_stock_by_ak('600009', '20130308', '20150413')
    ...
    # 计算买入卖出信号
    stock_df['AMA_shift'] = stock_df.shift(1)
    stock_df = stock_df.drop(range(AMA_length + 1), axis=0).reset_index(drop=True)
    stock_df['signal'] = np.where(stock_df['AMA'] - stock_df['AMA_shift'] > stock_df['filter'], 1, 0)
    stock_df['signal'] = np.where((stock_df['AMA_shift'] - stock_df['AMA']) > stock_df['filter'], -1, 0)
    ...
    for i in range(len(stock_df)):
        if stock_df['signal'] > 0 and current_stock_in_hand == 0:
